    ...
    case 0x5105:
      {
        //for(i = 0; i < 4; i++)
        {
		  ...
		  ppu->set_name_table(8, data & 0x03);
		  ppu->set_name_table(9, (data >> 2) & 0x03);
		  ppu->set_name_table(10, (data >> 4) & 0x03);
		  ppu->set_name_table(11, data >> 6);
          //set_VRAM_bank(8+0, data & 0x03);
          //data >>= 2;
		  //set_VRAM_bank(8+1, (data >> 2) & 0x03);
		  //set_VRAM_bank(8+2, (data >> 4) & 0x03);
		  //set_VRAM_bank(8+3, data >> 6);
        }
      }
      break;
    ...
    case 0x5107:
      {
        ...
        //  nametable3[i] = data;
        //}
		memset(nametable3 + 0x3C0, data, 0x400 - 0x3C0);
      }
      break;
    ...
    case 0x512B:
      {
		...
		// workaround for eVC3's arm compiler bug :(
		// Rick, 2003.4
		switch(addr & 0x03) {
		case 0:
			chr_reg[0][1] = data;
...
void NES_mapper5::sync_Chr_banks(uint8 mode)
{
  if(chr_size == 0)
  {
	...
	*/
	int base = chr_reg[7][mode]*8;
	set_PPU_banks(base + 0, base + 1, base + 2, base + 3, 
  ...
  else if(chr_size == 1)
  {
	...
	*/
	int base1 = chr_reg[3][mode]*4;
	int base2 = chr_reg[7][mode]*4;
	set_PPU_banks(base1 + 0, base1 + 1, base1 + 2, base1 + 3, 
  ...
  else if(chr_size == 2)
  {
	...
	*/
	set_PPU_banks(chr_reg[1][mode]*2+0, chr_reg[1][mode]*2+1,
		chr_reg[3][mode]*2+0, chr_reg[3][mode]*2+1,
		chr_reg[5][mode]*2+0, chr_reg[5][mode]*2+1,
		chr_reg[7][mode]*2+0, chr_reg[7][mode]*2+1);
  ...
  else
  {
	...
	*/
    set_PPU_banks(chr_reg[0][mode], chr_reg[1][mode], chr_reg[2][mode], chr_reg[3][mode],
