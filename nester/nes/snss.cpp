...
static void adopt_VRAM(SnssVramBlock* block, NES* nes)
{
  ...
  for(i = 0; i < 8; i++)
  {
	...
	// Rick
	CompiledTiles::compile(1024 / 16, &block->vram[i*0x400], nes->ppu->PPU_tile_banks[i]);
  }
...
static void adopt_MPRD(SnssMapperBlock* block, NES* nes)
{
  ...
  if(nes->ROM->get_num_8k_VROM_banks())
  {
	...
	// Rick
	uint8 * tiles = nes->ROM->get_VROM_tiles();
	nes->ppu->set_tile_banks(tiles + block->chrPages[0] * 0x400 / 16 * BYTES_PER_MERGED_TILE,
		tiles + block->chrPages[1] * 0x400 / 16 * BYTES_PER_MERGED_TILE,
		tiles + block->chrPages[2] * 0x400 / 16 * BYTES_PER_MERGED_TILE,
		tiles + block->chrPages[3] * 0x400 / 16 * BYTES_PER_MERGED_TILE,
		tiles + block->chrPages[4] * 0x400 / 16 * BYTES_PER_MERGED_TILE,
		tiles + block->chrPages[5] * 0x400 / 16 * BYTES_PER_MERGED_TILE,
		tiles + block->chrPages[6] * 0x400 / 16 * BYTES_PER_MERGED_TILE,
		tiles + block->chrPages[7] * 0x400 / 16 * BYTES_PER_MERGED_TILE);
  }
